...
/// Convert an Anthropic-style non-streaming response body into the SSE event
/// stream the client now consumes (it sends `Accept: text/event-stream` and
/// parses `message_start` → `content_block_*` → `message_delta` → `message_stop`
/// frames). The mocks keep declaring the response as a single JSON value; this
/// re-emits it as the equivalent stream so the tests survived the streaming
/// refactor without changing what each test asserts.
...
/// Build a 200 SSE `ResponseTemplate` from an Anthropic-style response body.
...
/// Test Anthropic completion request and response parsing
#[tokio::test]
async fn test_anthropic_completion_request_response() {
    ...
    let client = sombrax_agentic_core::providers::AnthropicClientBuilder::new("test-api-key")
    ...
    assert_eq!(response.message.content, "Hello! I'm Claude.");
    assert_eq!(response.usage.prompt_tokens, 12);
    assert_eq!(response.usage.completion_tokens, 8);
    assert_eq!(response.usage.total_tokens, 20);
}

/// Test Anthropic completion with sampling parameters
#[tokio::test]
async fn test_anthropic_with_sampling_params() {
    let mock_server = MockServer::start().await;
...
#[tokio::test]
async fn test_anthropic_completion_with_tool_calls() {
    let mock_server = MockServer::start().await;
...
/// Test Anthropic completion with thinking/reasoning content
#[tokio::test]
async fn test_anthropic_completion_with_thinking() {
    let mock_server = MockServer::start().await;
...
/// Test Anthropic response with cache usage metrics
#[tokio::test]
async fn test_anthropic_completion_with_cache_usage() {
    let mock_server = MockServer::start().await;
...
async fn test_anthropic_auth_error() {
    ...
    let client = sombrax_agentic_core::providers::AnthropicClientBuilder::new("invalid-key")
        .base_url(&mock_server.uri())
        .build();
...
/// The Anthropic API requires all tool_result blocks for a multi-tool-call
/// assistant message to appear in one user message immediately after.
#[tokio::test]
async fn test_anthropic_merges_consecutive_tool_results() {
    let mock_server = MockServer::start().await;
    ...
    let request = CompletionRequest {
        ...
        messages: vec![
            ...
            // Two separate tool result messages (as the agent framework produces)
            Message {
                role: "user".to_string(),
    ...
    // "tool_use.id 'call_2' was found without a corresponding tool_result block immediately after"
    let response = model.completion(request).await.unwrap();
    assert_eq!(response.message.content, "Based on the results...");

    // Verify the request was made (mock expects exactly 1 call)
    // The key assertion is that the request succeeded — if tool results weren't merged,
    // a real Anthropic API would return a 400 error.

    // Additionally verify via the received request body
